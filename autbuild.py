...
class Application(Frame):
    def createWidgets(self):
        ...
        self.cmdaut.set(
            " setmqaut -m QMGR -n 'NAME.OF.THE.QUEUE.OR.CHANNEL' -t queue -g mqm +browse +chg +clr +dlt +dsp +get +inq +put +passall +passid +set +setall +setid ")
        ...
        self.clstaffCheckBtn = ttk.Checkbutton(self.grpsLabelFrame, var=self.clstaffcb, onvalue='clstaff',
                                               offvalue='off')
        self.clstaffCheckBtn["text"] = "clstaff"
        self.clstaffCheckBtn.grid(row=0, column=3, padx=5, pady=5, sticky='W')
    ...
    def check_boxes(self):
        ...
        for i in self.chkbtnlist:
            self.CheckBtn = ttk.Checkbutton(self.grpsLabelFrame, var=i, onvalue=i, offvalue='off')
            self.CheckBtn["text"] = i
            if i == 'mqm':
                self.CheckBtn["state"] = "disabled"
            self.CheckBtn.grid(row=0, column=c, padx=5, pady=5, sticky='W')
            c += 1
    ...
    def read_queueAut(self):
        ...
        if os.path.isfile("queueaut.ini"):
    ...
    def readMqsc(self):
        ...
        if os.path.isfile(self.mqsc.get()):
            ...
            for t in file_str:
                ...
                if mqueue:
                    print mqueue.group(1)
                    print mqueue.group(2)
                    print mqueue.group(3)
            ...
            if not os.path.isfile(self.mqsc.get()):
        ...
        if os.path.isdir(self.output.get()):
            if not os.path.isfile(outputloc):
                ...
                if result == 'yes':
                    ...
                    return
        elif not os.path.isdir(self.output.get()):
    ...
    def createCommand(self):
        ...
        for g in self.grplist:
            if self.objTypeVar.get() == 'queue':
                # if user group mqm or broker give full authorities else basic authorities
